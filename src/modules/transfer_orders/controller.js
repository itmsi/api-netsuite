...
const getList = async (req, res) => {
  try {
    ...
    const syncInfo = await syncService.getLatestSyncInfo('transfer_orders').catch(() => null);
...
const create = async (req, res) => {
  try {
    ...
    const result = await service.createTransferOrder(req.body, req.user, userId);
...
const update = async (req, res) => {
  try {
    ...
    const result = await service.updateTransferOrder(req.body, req.user, userId);
    return res.status(200).json({
      success: true,
      data: {
        success: true,
