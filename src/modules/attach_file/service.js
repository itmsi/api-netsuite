...
const getPurchaseOrderById = async (id) => {
  const record = await dbNetsuite("purchase_orders").where("id", id).first();
  return record;
};
...
const callBridgeCreate = async ({
  ...
  try {
    ...
    console.error(
      "[AttachFile] Bridge create error:",
...
const callBridgeUpdate = async ({
  ...
  try {
    ...
    console.error(
      "[AttachFile] Bridge update error:",
      error?.response?.data || error.message,
    );
    // Non-blocking: log but don't throw
  }
...
const callBridgeDelete = async (netsuiteFileId, netsuiteId, type) => {
  try {
    ...
    console.error(
      "[AttachFile] Bridge delete error:",
      error?.response?.data || error.message,
    );
    // Non-blocking: log but don't throw
  }
