...
/**
 * Get project segmentations dari DB Netsuite (bridge_sanbox.project_segmentations)
 */
const getProjectSegmentationsList = async (body) => {
  try {
    ...
    const validSortColumns = [
      'netsuite_id', 'name', 'full_name', 'parent_id', 'parent_name',
    ...
    let query = dbNetsuite('project_segmentations').where('is_delete', false).where('is_inactive', false);
    ...
    if (body.parent_id) {
      query = query.where(function () {
        this.where('netsuite_id', body.parent_id.toString())
          .orWhere('parent_id', body.parent_id.toString());
      });
    }
    ...
    throw { message: error.message || 'Failed to fetch project segmentations from database', statusCode: 500 };
  }
};

/**
 * Sync project segmentations — hit bridge API (proses lama), return data langsung.
 */
const syncProjectSegmentationsList = async (body) => {
  try {
    ...
    const url = `${baseUrl}/api/v1/bridge/project-segmentation/get`;
    ...
    if (body.parent_id) {
      filters.parent_id = body.parent_id;
    }
    ...
    if (error.response) {
      throw {
        message: error.response.data.message || 'Failed to sync project segmentations from bridge API',
...
/**
 * Memproses sync ke tabel project_segmentations di gate_sso
 */
const processProjectSegmentationSync = async (records) => {
  ...
  try {
    for (const record of records) {
      ...
      const existing = await trx('project_segmentations').where('netsuite_id', data.netsuite_id.toString()).first();
      if (existing) {
        await trx('project_segmentations').where('netsuite_id', data.netsuite_id.toString()).update(data);
      } else {
        data.created_at = db.fn.now();
        await trx('project_segmentations').insert(data);
      }
    }
    await trx.commit();
  } catch (error) {
    await trx.rollback();
    console.error('Error syncing project segmentations to gate_sso:', error);
    throw error;
  }
};

module.exports = {
  getProjectSegmentationsList,
  syncProjectSegmentationsList,
  processProjectSegmentationSync
};
