...
const methodExecution = async (payload, channel, msg) => {
  ...
  try {
    ...
    if (eventStatus === "FAILED" || eventStatus === "SUCCESS") {
      console.info(
        `[Worker] PO Event ${event_id} already ${eventStatus}, skipping (ACK to clear stale DLQ message)`,
      );
  ...
  try {
    ...
    if (result && (result.success || result.poId || result.po_id)) {
      ...
      if (files.length > 0) {
        ...
        if (tempPoId && poId) {
          console.info(
            `[Worker] Found temporary PO ID: ${tempPoId}, finalizing files to real PO ID: ${poId}`,
          );
          try {
            await purchasingService.finalizeUploadedFilesForPO(tempPoId, poId);
          } catch (finalizeErr) {
            console.error(
              `[Worker] Error finalizing uploaded files for PO:`,
              finalizeErr.message,
            );
          }
        }
    ...
    try {
      ...
      console.error(
        `[Worker] Error processing PO Event ${event_id}:`,
        errorMessage,
      );
      ...
      if (allowRetry) {
        ...
        console.info(
          `[Worker] Retrying PO Event ${event_id} (retry_count: ${updated?.retry_count}/${updated?.max_retry})`,
        );
        await purchasingService.logEvent(
          event_id,
          "retry",
        ...
        console.error(
          `[Worker] Max retries reached for PO Event ${event_id}, marking as FAILED (manual retry required)`,
        );
      ...
      console.error(
        `[Worker] Failed to handle retry logic for PO Event ${event_id}:`,
        retryErr.message,
      );
...
const initPurchaseOrderServices = async () => {
  ...
  try {
    ...
    console.info(
      `[Worker] Purchase Order listener initialized on queue: ${queueName}`,
    );
  } catch (error) {
    console.error(
      "[Worker] Failed to initialize Purchase Order listener:",
      error,
    );
  }
};
