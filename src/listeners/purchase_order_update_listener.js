...
const methodExecution = async (payload, channel, msg) => {
  ...
  try {
    ...
    if (eventStatus === "FAILED" || eventStatus === "SUCCESS") {
      console.info(
        `[Worker] PO Update Event ${event_id} already ${eventStatus}, skipping (ACK to clear stale DLQ message)`,
      );
  ...
  try {
    ...
    if (result && (result.success || result.poId || result.po_id)) {
      const poId =
        result.poId || result.po_id || (result.data && result.data.id);
      ...
      await purchasingService.logEvent(
        event_id,
        "success",
        "Purchase order updated successfully in NetSuite",
        result,
      );
      ...
      if (files.length > 0) {
        if (poId) {
          console.info(
            `[Worker] Found PO ID: ${poId}, finalizing files to correct folder if they are in /Temp/`,
          );
          try {
            ...
            console.error(
              `[Worker] Error finalizing uploaded files for PO Update:`,
              finalizeErr.message,
            );
          }
        }
      ...
      if (poId) {
        console.info(
          `[Worker] Triggering sync for PO ID after update: ${poId}`,
        );
        try {
          //await purchasingService.syncPurchaseOrderByIdInternalId(poId, po_internal_id);
          await purchasingService.logEvent(
            event_id,
            "purchase_order_synced",
            "Purchase order data synced to local DB",
            { po_id: poId },
          );
        } catch (syncError) {
          console.error(
            `[Worker] Sync failed for PO ID ${poId}:`,
            syncError.message,
          );
          await purchasingService.logEvent(
            event_id,
    ...
    try {
      ...
      console.error(
        `[Worker] Error processing PO Update Event ${event_id}:`,
        errorMessage,
      );
      ...
      if (allowRetry) {
        ...
        console.info(
          `[Worker] Retrying PO Update Event ${event_id} (retry_count: ${updated?.retry_count}/${updated?.max_retry})`,
        );
        await purchasingService.logEvent(
          event_id,
          "retry",
        ...
        console.error(
          `[Worker] Max retries reached for PO Update Event ${event_id}, marking as FAILED (manual retry required)`,
        );
        ...
        const poRecord =
          await purchasingService.getPurchaseOrderById(po_internal_id);
        poId = poRecord?.data?.po_id || null;

        if (poId) {
          console.info(
            `[Worker] Triggering sync for PO ID after update failure: ${poId}`,
          );
          try {
            await purchasingService.syncPurchaseOrderByIdInternalId(
              poId,
              po_internal_id,
            );
            await purchasingService.logEvent(
              event_id,
              "purchase_order_synced",
              "Purchase order data synced to local DB after update failure",
              { po_id: poId },
            );
          } catch (syncError) {
            console.error(
              `[Worker] Sync failed for PO ID ${poId} after update failure:`,
              syncError.message,
            );
            await purchasingService.logEvent(
              event_id,
      ...
      console.error(
        `[Worker] Failed to handle retry logic for PO Update Event ${event_id}:`,
        retryErr.message,
      );
...
const initPurchaseOrderUpdateServices = async () => {
  ...
  try {
    ...
    console.info(
      `[UpdateWorker] Purchase Order update listener initialized on queue: ${queueName}`,
    );
  } catch (error) {
    console.error(
      "[UpdateWorker] Failed to initialize Purchase Order update listener:",
      error,
    );
  }
};
