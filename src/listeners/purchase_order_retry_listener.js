...
const methodExecution = async (payload, channel, msg) => {
  ...
  try {
    ...
    if (eventStatus === "FAILED" || eventStatus === "SUCCESS") {
      console.info(
        `[RetryWorker] PO Event ${event_id} already ${eventStatus}, skipping`,
      );
      return channel.ack(msg);
    }
  } catch (guardErr) {
    console.warn(
      `[RetryWorker] Could not check event status for ${event_id}:`,
      guardErr.message,
    );
  }
  ...
  try {
    console.info(
      `[RetryWorker] Processing manual retry for Event ID: ${event_id}`,
    );

    await purchasingService.logEvent(
      event_id,
      "processing",
      "Manual retry: starting request to bridge API",
      data,
    );

    const dataWithInternalId = { ...data, internalid: po_internal_id };
    const result =
      await purchasingService.createPurchaseOrderToBridge(dataWithInternalId);
    ...
    if (result && (result.success || result.poId || result.po_id)) {
      ...
      await purchasingService.logEvent(
        event_id,
        "success",
        "Manual retry: purchase order created successfully",
        result,
      );
      ...
      if (poId) {
        ...
        try {
          ...
          await purchasingService.logEvent(
            ...
            "Manual retry sync successful",
            { po_id: poId },
          );
        } catch (syncError) {
          console.error(
            `[RetryWorker] Sync failed for PO ID ${poId}:`,
            syncError.message,
          );
          await purchasingService.logEvent(
            event_id,
      ...
      throw new Error(
        result.message || "Bridge API returned failure on manual retry",
      );
    }
    ...
    try {
      ...
      console.error(
        `[RetryWorker] Error processing manual retry for PO Event ${event_id}:`,
        errorMessage,
      );
      ...
      await purchasingService.logEvent(
        event_id,
        "failed",
        `Manual retry failed: ${errorMessage}`,
        errorDetail,
      );
      await purchasingService.updateLocalPOStatus(po_internal_id, "failed");
    } catch (updateErr) {
      console.error(
        `[RetryWorker] Failed to update status for PO Event ${event_id}:`,
        updateErr.message,
      );
    }
...
const initPurchaseOrderRetryServices = async () => {
  ...
  try {
    ...
    console.info(
      `[RetryWorker] Purchase Order manual retry listener initialized on queue: ${queueName}`,
    );
  } catch (error) {
    console.error(
      "[RetryWorker] Failed to initialize Purchase Order retry listener:",
      error,
    );
  }
};
