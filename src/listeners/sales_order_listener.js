...
const { EXCHANGES, QUEUE, logger, todayFormat } = require("../utils");
...
const methodExecution = async (payload, channel, msg) => {
  ...
  try {
    ...
    if (eventStatus === "FAILED" || eventStatus === "SUCCESS") {
      console.info(
        `[Worker] SO Event ${event_id} already ${eventStatus}, skipping (ACK to clear stale DLQ message)`,
      );
  ...
  try {
    ...
    const result =
      await salesService.createSalesOrderToBridge(dataWithInternalId);

    if (
      result &&
      (result.success || result.soId || result.so_id || result.data?.id)
    ) {
      const soId =
        result.soId || result.so_id || result.data?.id || result.data?.soId;
      ...
      if (files.length > 0) {
        ...
        if (tempSoId && soId) {
          console.info(
            `[Worker] Found temporary SO ID: ${tempSoId}, finalizing files to real SO ID: ${soId}`,
          );
          try {
            await salesService.finalizeUploadedFilesForSO(tempSoId, soId);
          } catch (finalizeErr) {
            console.error(
              `[Worker] Error finalizing uploaded files for SO:`,
              finalizeErr.message,
            );
          }
        }
      ...
      if (soId) {
        ...
        try {
          ...
          console.error(
            `[Worker] Sync failed for SO ID ${soId}:`,
            syncError.message,
          );
          await salesService.logEvent(
            event_id,
    ...
    try {
      ...
      console.error(
        `[Worker] Error processing SO Event ${event_id}:`,
        errorMessage,
      );
      ...
      if (allowRetry) {
        const updated = await salesService.incrementRetryCount(
          event_id,
          errorMessage,
        );
        console.info(
          `[Worker] Retrying SO Event ${event_id} (retry_count: ${updated?.retry_count}/${updated?.max_retry})`,
        );
        await salesService.logEvent(
          event_id,
          "retry",
        ...
        console.error(
          `[Worker] Max retries reached for SO Event ${event_id}, marking as FAILED`,
        );
        await salesService.updateLocalSalesOrderStatus(
          so_internal_id,
          "failed",
        );

        const failureProperties = { request: data, response: errorDetail };
        await salesService.updateEventStatus(
          event_id,
          "FAILED",
          errorMessage,
          failureProperties,
      ...
      console.error(
        `[Worker] Failed to handle retry logic for SO Event ${event_id}:`,
        retryErr.message,
      );
      channel.ack(msg);
    }
...
const initSalesOrderServices = async () => {
  ...
  try {
    ...
    console.info(
      `[Worker] Sales Order listener initialized on queue: ${queueName}`,
    );
  } catch (error) {
